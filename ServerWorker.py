			...
			if data: 
				...
				try:
					...
					while offset < frame_len:
						end = min(offset + MAX_RTP_PAYLOAD, frame_len)
						payload = frame[offset:end]
						# marker bit 1 only for last fragment
						marker = 1 if end == frame_len else 0
						# increment sequence number per fragment
						self.clientInfo['seq'] += 1
						packet = self.makeRtp(payload, self.clientInfo['seq'], marker)
